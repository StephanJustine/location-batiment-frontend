...
export default function PaiementCalendar({ bailId, loyerMensuel, chargesMensuelles, onRefresh }: Props) {
  ...
  const [paiementsAssocies, setPaiementsAssocies] = useState<Map<number, number>>(new Map());
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      // Récupérer les paiements associés aux sessions pour les quittances
      const allPaiements = await paiementService.getAll({ bail_id: bailId, limit: 200 });
      const paiementsMap = new Map();
      if (Array.isArray(allPaiements)) {
        allPaiements.forEach((p: any) => {
          if (p.session_id) {
            paiementsMap.set(p.session_id, p.id);
          }
        });
      }
      setPaiementsAssocies(paiementsMap);
    } catch (err: any) {
...
const handleGenerateQuittance = async (session: PaiementSession) => {
  if (!session || session.statut !== 'paye') {
    setError('Seules les sessions payées peuvent générer une quittance');
    return;
  }
  
  const paiementId = paiementsAssocies.get(session.id);
  
  if (!paiementId) {
    setError('Aucun paiement trouvé pour cette session');
    return;
  }
  
  setGeneratingQuittance(session.id);
  try {
    // 🔥 Utiliser le service qui gère déjà l'authentification
    ...
    if (err?.response?.status === 401) {
      ...
      setError(err?.message || 'Erreur lors de la génération de la quittance');
    }
  } finally {
  ...
  const getStatutIcon = (statut: string) => {
    switch (statut) {
      case 'paye': return <CheckCircle sx={{ fontSize: 18, color: '#22c55e' }} />;
      case 'en_retard': return <Warning sx={{ fontSize: 18, color: '#ef4444' }} />;
      case 'partiel': return <AttachMoney sx={{ fontSize: 18, color: '#f59e0b' }} />;
      default: return <Schedule sx={{ fontSize: 18, color: '#94a3b8' }} />;
    }
  };
  ...
  if (sessions.length === 0 && !loading) {
    return (
      <Box sx={{ textAlign: 'center', py: 6 }}>
        ...
        <Button variant="contained" onClick={() => setOpenGenerateDialog(true)} startIcon={<Autorenew />} sx={{ mt: 2 }}>
        ...
        <Dialog open={openGenerateDialog} onClose={() => setOpenGenerateDialog(false)} maxWidth="sm" fullWidth>
          ...
          <DialogActions>
            ...
            <Button variant="contained" onClick={handleGenerateEcheances} disabled={generating} sx={{ bgcolor: '#059669' }}>
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <IconButton onClick={fetchSessions} size="small" sx={{ bgcolor: '#f1f5f9' }}>
          <Refresh />
        </IconButton>
      </Box>
      ...
      <Grid container spacing={1.5}>
        {sessions.map((session) => {
          ...
          const aUneQuittance = estComplet && paiementsAssocies.has(session.id);
            ...
            <Grid size={{ xs: 6, sm: 4, md: 3, lg: 2 }} key={session.id}>
              <Card sx={{ 
                ...
                <CardContent sx={{ p: 1.5, textAlign: 'center' }}>
                  ...
                  <LinearProgress variant="determinate" value={pourcentagePaye} sx={{ height: 3, borderRadius: 2, my: 1 }} />
                  ...
                  <Chip
                    label={estComplet ? 'Payé' : `${Math.round(pourcentagePaye)}%`}
                    size="small"
                  ...
                  {!estComplet && reste > 0 && (
                    <Button
                      ...
                      sx={{ mt: 1, fontSize: '0.6rem', py: 0.3, bgcolor: statusColor }}
                    >
                      Payer
                    </Button>
                  )}
                  
                  {estComplet && aUneQuittance && (
                    <Button
                      size="small"
                      variant="outlined"
                      startIcon={generatingQuittance === session.id ? <CircularProgress size={14} /> : <PictureAsPdf sx={{ fontSize: 14 }} />}
                      onClick={() => handleGenerateQuittance(session)}
                      disabled={generatingQuittance === session.id}
                      sx={{ mt: 1, fontSize: '0.6rem', py: 0.3 }}
                    >
                      {generatingQuittance === session.id ? 'Génération...' : 'Quittance'}
                    </Button>
                  )}
                </CardContent>
              </Card>
      ...
      <Dialog open={openPayDialog} onClose={() => setOpenPayDialog(false)} maxWidth="sm" fullWidth>
          ...
          <Box sx={{ mt: 2 }}>
            ...
            <TextField
              ...
              onChange={(e) => {
                ...
                if (value <= restant) setPayData({ ...payData, montant: value });
              }}
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handlePaySession} disabled={submitting} sx={{ bgcolor: '#059669' }}>
