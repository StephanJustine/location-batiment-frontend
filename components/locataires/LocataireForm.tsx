...
interface Props { 
  ...
  onSubmit: (d: FD, garants: Garant[]) => Promise<void>; 
  onCancel: () => void; 
  loading?: boolean; 
}
...
const STEPS = [
  ...
  { label: 'Garants', icon: <FamilyRestroom />, c: '#e91e63' }
];
...
const FIELDS: Record<number, (keyof FD)[]> = {
  ...
  3: ['situation_matrimoniale', 'nombre_enfants', 'nombre_personnes', 'notes']
};

const clean = (d?: Partial<FD>): Partial<FD> => d ? {
  ...d,
...
export default function LocataireForm({ initialData, onSubmit, onCancel, loading = false }: Props) {
  ...
  const { control, handleSubmit, formState: { errors }, trigger } = useForm<FD>({
    ...
    defaultValues: {
      nom: init.nom || '',
      prenom: init.prenom || '',
      nationalite: init.nationalite || 'MALAGASY',
      telephone: init.telephone || '',
      nombre_enfants: init.nombre_enfants ?? 0,
      nombre_personnes: init.nombre_personnes ?? 1,
      // date_naissance: init.date_naissance || null,
      date_naissance: initialData?.date_naissance?.split('T')[0] || '', // Format YYYY-MM-DD pour l'affichage
      lieu_naissance: init.lieu_naissance || null,
      cin: init.cin || null,
      passeport: init.passeport || null,
      email: init.email || null,
      telephone_secondaire: init.telephone_secondaire || null,
      adresse: init.adresse || null,
      profession: init.profession || null,
      employeur: init.employeur || null,
      revenu_mensuel: init.revenu_mensuel ?? null,
      situation_matrimoniale: init.situation_matrimoniale || '',
  ...
  const handleFormSubmit = async (data: FD) => {
    setSubmitError(null);
    try {
      await onSubmit(data, garants);
    } catch (error: any) {
      console.error('Erreur:', error);
      setSubmitError(error.response?.data?.detail || error.message || 'Une erreur est survenue');
    }
  };
      ...
      <Box key={f} sx={{ flex: full ? '1 1 100%' : '1 1 calc(50% - 8px)', minWidth: 250 }}>
        <Controller name={f} control={control} render={({ field: fl }) => {
          ...
          if (f === 'nationalite') return (
            <TextField {...fl} fullWidth select label={L[f]} value={fl.value || 'MALAGASY'} size="medium" error={!!errors[f]} helperText={errors[f]?.message}>
          ...
          if (f === 'situation_matrimoniale') return (
            <TextField {...fl} fullWidth select label={L[f]} value={fl.value || ''} size="medium" error={!!errors[f]} helperText={errors[f]?.message}>
          ...
          if (f === 'date_naissance') return (
            <TextField {...fl} fullWidth type="date" label={L[f]} value={fl.value || ''} size="medium" slotProps={{ inputLabel: { shrink: true } }} error={!!errors[f]} helperText={errors[f]?.message} />
          );
          // Revenu
          ...
          if (f === 'nombre_enfants') return (
            <TextField {...fl} fullWidth type="number" label={L[f]} value={fl.value ?? 0} size="medium" onChange={e => fl.onChange(Number(e.target.value) || 0)} slotProps={{ htmlInput: { min: 0, max: 20 } }} error={!!errors[f]} helperText={errors[f]?.message} />
          );
          // Nombre personnes
          if (f === 'nombre_personnes') return (
            <TextField {...fl} fullWidth type="number" label="Nombre de personnes *" value={fl.value ?? 1} size="medium" onChange={e => fl.onChange(Number(e.target.value) || 1)} slotProps={{ htmlInput: { min: 1, max: 10 } }} error={!!errors[f]} helperText={errors[f]?.message} />
          );
          // Email
          if (f === 'email') return (
    ...
    <Box sx={{ maxWidth: 900, mx: 'auto' }}>
      ...
      <Box component="form">
          ...
          <Card elevation={0} sx={{ borderRadius: 2, border: '1px solid #e0e0e0' }}>
            <CardContent sx={{ p: 3 }}>
              ...
              {step < 4 ? (
                ...
                <GarantForm garants={garants} onUpdate={setGarants} />
              )}
            </CardContent>
          </Card>
        </Fade>
        ...
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 3 }}>
          <Button onClick={onCancel} variant="outlined" startIcon={<ArrowBack />} disabled={loading}>
            Annuler
          </Button>
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <Typography variant="body2" color="text.secondary" sx={{ mr: 1 }}>{step + 1}/{STEPS.length}</Typography>
            {step > 0 && (
              <Button onClick={prev} variant="outlined" startIcon={<ArrowBack />} disabled={loading}>
                Précédent
              </Button>
            )}
            {step < 4 ? (
              <Button onClick={next} variant="contained" endIcon={<ArrowForward />} disabled={loading}>
                Suivant
              </Button>
            ) : (
              <Button 
