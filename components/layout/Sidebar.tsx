...
//     children: [
//       { text: 'Profil', icon: <PersonAddIcon />, path: '/parametres/profil' },
//       { text: 'Utilisateurs', icon: <PeopleIcon />, path: '/parametres/utilisateurs' },
//       { text: 'Tarifs', icon: <AttachMoney />, path: '/parametres/tarifs' },
//       { text: 'Sécurité', icon: <SecurityIcon />, path: '/parametres/securite' },
//     ]
//   },
...
import PersonAddIcon from '@mui/icons-material/PersonAdd';
import SecurityIcon from '@mui/icons-material/Security';
import VisibilityIcon from '@mui/icons-material/Visibility';  // ✅ Pour superviseur
import Link from 'next/link';
import { usePathname, useRouter } from 'next/navigation';
import { useAuth } from '@/contexts/AuthContext';
...
// ✅ Configuration des permissions par rôle avec SUPERVISEUR
const rolePermissions = {
  [UserRole.ADMIN]: {
    ...
    canManageEmployes: true,
    canManageTravaux: true,
    canManageBiens: true,
    canManageLocataires: true,
    canManageReleves: true,
    canViewReports: true,
    canApprove: true,
  },
  ...
  [UserRole.SUPERVISEUR]: {  // ✅ Nouveau rôle SUPERVISEUR
    canViewAll: true,
    canManageUsers: false,
    canManageSettings: false,
    canManageFinances: false,
    canManageEmployes: false,
    canManageTravaux: false,
    canManageBiens: false,
    canManageLocataires: false,
    canManageReleves: false,
    canViewReports: true,
    canApprove: false,
  },
  [UserRole.LOCATAIRE]: {
...
// ✅ Définition du menu avec permissions
const menuItems: MenuItem[] = [
  ...
  { 
    text: 'Locataires', 
    icon: <PeopleIcon />,
    permission: 'canManageLocataires',
    children: [
      { text: 'Locataires', icon: <PeopleIcon />, path: '/locataires' },
      { text: 'Baux', icon: <ReceiptIcon />, path: '/baux' },
      { text: 'Contrats', icon: <DescriptionIcon />, path: '/contrats' },
...
export default function Sidebar({ mobileOpen, onClose }: SidebarProps) {
  ...
  const isActive = (path?: string) => path ? pathname === path || pathname.startsWith(path + '/') : false;
  const isParentActive = (children: MenuItem[]) => children.some(child => isActive(child.path));
  ...
  const roleConfig = {
    [UserRole.ADMIN]: { label: 'Admin', color: '#dc2626', bg: '#fef2f2' },
    [UserRole.GESTIONNAIRE]: { label: 'Gestionnaire', color: '#2563eb', bg: '#eff6ff' },
    [UserRole.SUPERVISEUR]: { label: 'Superviseur', color: '#7c3aed', bg: '#f3e8ff' },
    [UserRole.LOCATAIRE]: { label: 'Locataire', color: '#059669', bg: '#f0fdf4' }
  };
  ...
  const renderMenuItem = (item: MenuItem) => {
    ...
    if (item.children) {
      const accessibleChildren = item.children.filter((child: MenuItem) => {
        if (child.permission) {
          return hasPermission(child.permission);
        }
        return item.permission ? hasPermission(item.permission) : true;
      ...
      const isOpen = openMenus[item.text] || isParentActive(accessibleChildren);
      return (
        <Box key={item.text}>
          <ListItemButton
            onClick={() => handleMenuClick(item.text)}
          ...
          {!collapsed && (
            <Collapse in={isOpen} timeout="auto">
              <List disablePadding>
                {accessibleChildren.map((child: MenuItem) => (
                  <ListItemButton
                    key={child.text}
                    component={Link}
                    href={child.path || '#'}
    ...
    // ✅ Item simple - vérifier que path existe
